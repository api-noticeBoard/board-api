...
public class PostController {
    ...
    public ResponseEntity<Void> restorePost(@Parameter(description = "복원할 ID들(쉼표구분)", example = "1, 2, 3")
                                            @RequestParam(value = "postIds") String postIds) {
        List<Long> postIdList;

        try {
            postIdList = Arrays.stream(postIds.trim().split(","))
                    .map(String::trim)
                    .map(Long::parseLong)
                    .toList();
        } catch (NumberFormatException e) {
            throw new BusinessException(ErrorCode.INVALID_INPUT_VALUE, ": ID 형식이 올바르지 않습니다.");
        }

        postService.restorePost(postIdList);
    ...
    @Operation(summary = "게시글 임시 삭제", description = "게시글ID의 삭제")
    public ResponseEntity<Void> deletePost(@Parameter(description = "삭제할 ID들(쉼표구분)", example = "1, 2, 3")
                                           @RequestParam("postIds") String postIds) {
        List<Long> postIdList;
        try {
            postIdList = Arrays.stream(postIds.split(","))
                    .map(String::trim)
                    .map(Long::parseLong)
                    .toList();
        } catch (NumberFormatException e) {
        ...
        postService.softDeletePost(postIdList);
     ...
     */
    @DeleteMapping("/deletion/permanent")
    @Operation(summary = "게시글 영구 삭제 (관리자)", description = "게시글을 물리적으로 완전히 삭제합니다.")
    public ResponseEntity<Void> deletePostPermanently(@Parameter(description = "삭제할 ID들(쉼표구분)", example = "1, 2, 3")
                                                      @RequestParam("postIds") String postIds) {
        List<Long> postIdList;
        try {
            postIdList = Arrays.stream(postIds.trim().split(","))
        ...
        postService.hardDeletePost(postIdList);
        return ResponseEntity.noContent().build();
    }
